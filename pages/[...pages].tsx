...
export async function getStaticProps({ preview, params, locale, locales }: GetStaticPropsContext<{ pages: string[] }>) {
  ...
  const paths = await commerce.getAllBlogPaths();
...
export default function Pages({ page }: InferGetStaticPropsType<typeof getStaticProps>) {
  ...
  return router.isFallback ? (
    ...
    <div className="max-w-2xl mx-8 sm:mx-auto py-20">{page?.body && <div>{page?.body}</div>}</div>
  );
}
